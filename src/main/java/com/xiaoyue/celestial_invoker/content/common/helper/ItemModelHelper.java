    ...
    public static void createCrossbowModel(DataGenContext<?, ?> ctx, RegistrateItemModelProvider pvd) {
        ...
        for(int i = 0; i < 3; ++i) {
            ...
            if (CROSSBOW_PULL_VALS[i] > 0.0F) {
                override.predicate(ResourceLocation.withDefaultNamespace("pull"), CROSSBOW_PULL_VALS[i]);
            }
        ...
        builder.override().predicate(ResourceLocation.withDefaultNamespace("charged"), 1.0f)
        ...
        builder.override().predicate(ResourceLocation.withDefaultNamespace("firework"), 1.0f)
